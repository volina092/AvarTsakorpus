...
class Splitter:
    ...
    def add_speaker_marks(self, sentences):
        ...
        if 'insert_speaker_marks' in self.settings and not self.settings['insert_speaker_marks']:
            return
        langs2process = [i for i in range(len(self.settings['languages']))]
        if 'speaker_marks_languages' in self.settings:
            langs2process = [i for i in range(len(self.settings['languages']))
                             if self.settings['languages'][i] in self.settings['speaker_marks_languages']]
